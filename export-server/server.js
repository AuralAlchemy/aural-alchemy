...
app.post('/export', (req, res, next) => {
  ...
  // Parse fields from either body or form
  const body = req.body || {};
  ...
  const fadeIn     = parseFloat(body.fadeIn)   || 0; // seconds, 0 = disabled
...
app.post('/export', async (req, res) => {
  const { clipUrl, loopIn = 0, loopOut, duration = 60, speed = 0.5, resolution = '1080', email } = req.body;
  if (!clipUrl)    return res.status(400).json({ error: 'clipUrl required' });
  if (!email)      return res.status(400).json({ error: 'email required' });
  ...
  processExport({ jobId, clipUrl, loopIn, loopOut, duration, speed, resolution, email, tmpIn, tmpLoop, outFile });
});
...
async function processExport({ jobId, clipUrl, loopIn, loopOut, duration, speed, resolution, email, tmpIn, tmpLoop, outFile }) {
  const job = jobs[jobId];
  try {
    job.status = 'downloading'; job.progress = 5;
    await downloadFile(clipUrl, tmpIn);
    ...
    await runCmd(trimCmd, jobId);
    job.progress = 50;
    console.log(`[${jobId}] trim done, encoding ${duration}s at ${speed}x...`);
    ...
    if (email && RESEND_API_KEY) {
      await sendEmail(email, job.url, job.size);
    }
